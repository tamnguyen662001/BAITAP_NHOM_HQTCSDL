...
INSERT SAN_PHAM VALUES ('GK011', N'BIA HÀ NỘI', 1000,670000, N'THÙNG', 'GK03','#')
...
(@msp varchar(5), @tensp nvarchar(50), @sl tinyint, @dongia float,
@dvt nvarchar(15), @mlsp varchar(5),@anhsp nvarchar(100),
@mnl varchar(5))
as
begin
	INSERT CHI_TIET_SAN_PHAM VALUES ( @tensp,@mnl, 1)
	INSERT SAN_PHAM VALUES (@msp,@tensp, @sl,@dongia,@dvt, @mlsp,@anhsp)
end

exec insertfood 'GK013', N'BIA nha trang', 10,500000, N'THÙNG', 'GK03','#', NL07
...
INSERT HOA_DON VALUES ('HD06', '06','KH3', '20210409', '02')
INSERT HOA_DON VALUES ('HD07', '07','KH1', '20210419', '02')
INSERT HOA_DON VALUES ('HD08', '08','KH5', '20210429', '01')
...
INSERT CHI_TIET_HOA_DON VALUES(7,'HD06','HS000')
INSERT CHI_TIET_HOA_DON VALUES(3,'HD07','HS011')
INSERT CHI_TIET_HOA_DON VALUES(8,'HD08','TH022')
...
HAVING COUNT(C.MSP) BETWEEN 2 AND 5
...
AS
BEGIN
		SELECT * FROM NHAN_VIEN WHERE TENDN = @TENDN
END
...
AS
BEGIN
	SELECT * FROM NHAN_VIEN WHERE  TENDN= @TK AND MK = @MK
END
GO
exec login 'QUANLI','QUANLI'
...
AS
BEGIN
	SELECT B.TENSP AS N'TÊN MÓN' , A.SOLUONG AS N'SL' , B.DONGIA AS N'ĐƠN GIÁ' , SUM(A.SOLUONG*B.DONGIA) AS 'THÀNH TIỀN'
	FROM CHI_TIET_HOA_DON A JOIN SAN_PHAM B ON A.MSP = B.MSP
	WHERE A.MHD = @ID
	GROUP BY B.TENSP, A.SOLUONG, B.DONGIA
END
...
AS
BEGIN
	SELECT B.TENSP AS N'TÊN MÓN' , B.DONGIA AS N'ĐƠN GIÁ'
	FROM CHI_TIET_HOA_DON A JOIN SAN_PHAM B ON A.MSP = B.MSP
	WHERE A.MHD = @ID
	GROUP BY B.TENSP,  B.DONGIA
END
...
as
begin
	select * from SAN_PHAM where MLSP = @mlsp
end
...
as
begin
	select * from LOAI_SAN_PHAM where MLSP = @mlsp
end

exec lsp hs01

  select * from LOAI_SAN_PHAM where MLSP = 'CA01'
...
as
begin
	INSERT HOA_DON VALUES (@mhd, '04','KH1', GETDATE(), '04') 
end

select MSP as N'Mã SP' ,TENSP as N'Món', SOLUONG as N'SL', DVT as N'Đơn vị tính'  from SAN_PHAM

select TENNV, CHUCVU, TENDN, DIACHI from NHAN_VIEN
select * from NHAN_VIEN
update NHAN_VIEN set TENDN = '',TENNV = N'',NGAYSINH = '', SDT = '',GIOITINH = '',DIACHI = '', MK='', CHUCVU = '' where MNV = '07'
delete from NHAN_VIEN where MNV = '07'
